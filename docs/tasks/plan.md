...
* [x] 9.10 **Fix:** Normalize Memory URI keys to opaque safe IDs. Return `mem://v1/${safeKey}?sig=...` instead of exposing raw key in URI. Document URI path as opaque.
...
* [x] 9.14 **Docs:** Add Binary Supervisor example to docs — Evaluator writes `GateFinding[]` to Trace, returns binary decision, Supervisor consumes only decision, WorkflowRunner records `RouteDecision` to Trace.
